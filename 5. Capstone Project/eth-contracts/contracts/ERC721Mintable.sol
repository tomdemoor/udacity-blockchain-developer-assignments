...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(_owner == msg.sender, "Caller is not owner of contract");
        _;
    }
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        require(newOwner != address(0), "Please check address and retry");
        // add functionality to transfer control of the contract to a newOwner.
        address _oldOwner = _owner;
        _owner = newOwner;
        // Emit event when ownership is transfered
        emit TransferedOwnership(_oldOwner, newOwner);
    }
}
...
contract Pausable is Ownable {
    ...
    //  2) create a public setter using the inherited onlyOwner modifier -> THIS CANNOT BE DONE IN ONE FUNCTION
    // function setPauseStatus(bool _newStatus) onlyOwner {
    //     _paused = _newStatus;
    // }
    ...
    modifier whenNotPaused() {
        require(_paused == false, "Contract is paused, please reset it to continue");
        _;
    }
    modifier paused() {
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != ownerOf(tokenId), "Token owner not allowed");

        // require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(msg.sender == getOwner() || isApprovedForAll(msg.sender, to), "Not authorised to approve ownership transfer");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(from == ownerOf(tokenId), "From address is not owner of given tokenId");
        ...
        // emit correct event -> Transfer event ??
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function setTokenURI(uint256 tokenId) internal {
        ...
        require(_exists(tokenId), "Oops! Token tokenId doesnot exists");
...
contract TristarHomesERC721Token is ERC721Metadata {
    ...
    function mint(address to, uint256 tokenId) public onlyOwner returns(bool isMinted){
        _mint(to, tokenId);
        setTokenURI(tokenId);
        isMinted = true;
    }
